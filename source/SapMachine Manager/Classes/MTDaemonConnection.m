...
- (void)connectToXPCService
{
    ...
    if (! _xpcServiceConnection) {
        ...
        _xpcServiceConnection = [[NSXPCConnection alloc] initWithServiceName:@"corp.sap.SapMachineXPC"];
        [_xpcServiceConnection setRemoteObjectInterface:[NSXPCInterface interfaceWithProtocol:@protocol(SapMachineXPCProtocol)]];
                    ...
                    andExportObject:(id)exportedObject
{
    ...
    if (! _connection && endpoint) {
        ...
        NSXPCInterface *remoteObjectInterface = [NSXPCInterface interfaceWithProtocol:@protocol(SMUDaemonProtocol)];

        // allowed classes
        [remoteObjectInterface setClasses:[NSSet setWithObjects:[MTSapMachineAsset class], [NSArray class], nil]
                              forSelector:@selector(availableAssetsWithReply:)
                            argumentIndex:0
                                  ofReply:YES
        ];
        [remoteObjectInterface setClasses:[NSSet setWithObjects:[MTSapMachineAsset class], [NSArray class], nil]
        ...
        [_connection setExportedInterface:[NSXPCInterface interfaceWithProtocol:@protocol(MTSapMachineAssetUpdateDelegate)]];
